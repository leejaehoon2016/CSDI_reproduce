...
class DiffTrainer(nn.Module):
    def __init__(
        ...
        self.alpha_bar = torch.from_numpy(self.alpha_bar).float()
        self.n_samples = n_samples

    def forward(self, model, batch):
        observed_data = batch["observed_data"].permute(0, 2, 1).float()
        observed_mask = batch["observed_mask"].permute(0, 2, 1).float()
        observed_tp = batch["timepoints"].float()
        gt_mask = batch["gt_mask"].permute(0, 2, 1).float()

        t = torch.randint(0, self.num_steps, [len(observed_data),])
    ...
    def impute(self, model, batch):
        ...
        observed_mask = batch["observed_mask"].permute(0, 2, 1).float()
        observed_tp = batch["timepoints"].float()
        cond_mask = batch["gt_mask"].permute(0, 2, 1).float()
        ...
        for i in range(self.n_samples):
            ...
            for t in range(self.num_steps - 1, -1, -1):
                cond_data = (cond_mask * observed_data).unsqueeze(1)
